...
object Episodes {
  ...
  def episodes(id: Int)(implicit ctx: Context): IO[EpisodeRecordData] = withToken { token =>
    val req = GET(ctx.baseUri / "episodes" / id.toString, ctx.acceptHeader, ctx.acceptLanguageHeader,
